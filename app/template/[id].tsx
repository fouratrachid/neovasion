...
const getIncludeColor = (type: string) => {
  switch (type) {
    case "meal":
      return { bg: "#FFF7ED", icon: "#EA580C", text: "#9A3412" };
    case "transport":
      return { bg: "#EFF6FF", icon: "#2563EB", text: "#1D4ED8" };
    case "guide":
      return { bg: "#F0FDF4", icon: "#16A34A", text: "#15803D" };
    case "accommodation":
      return { bg: "#F5F3FF", icon: "#7C3AED", text: "#6D28D9" };
    default:
      return { bg: "#F0FDF4", icon: "#059669", text: "#047857" };
  }
};
...
export default function TemplateDetailScreen() {
  ...
  const { template, isLoading, error, refetch } = useTemplateDetails(
    id as string,
  );
          ...
          <View style={styles.metaRow}>
            {template.date_depart && (
              <View style={styles.metaChip}>
                ...
                <Text style={styles.metaText}>
                  {formatTripDate(template.date_depart)}
                </Text>
              </View>
            )}
            {totalDays > 0 && (
            ...
            {template.destination && template.destination.length > 0 && (
              <View style={styles.metaChip}>
                ...
                />
                <Text style={styles.metaText}>
                  {template.destination.length} Stops
                </Text>
              </View>
            )}
          </View>
                ...
                {template.includes.map((inc, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text
                        style={[styles.includeText, { color: colors.text }]}
                        numberOfLines={2}
                      >
                        {inc.value}
                      </Text>
      ...
      <View style={styles.ctaBar}>
        <View>
          ...
          <Text style={styles.ctaCity}>
            ...
            {template.destination?.[template.destination.length - 1]?.location
              ?.ville ?? ""}
          </Text>
        </View>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={styles.timelineDateRow}>
                    ...
                    <Text style={styles.timelineDate}>
                      {dayjs(dest.date_start).format("DD MMM")} →{" "}
                      {dayjs(dest.date_end).format("DD MMM YYYY")}
                    </Text>
                  </View>
                </View>
                {days != null && (
                      ...
                      <View key={hi} style={styles.hebCard}>
                        ...
                        <View style={styles.hebInfo}>
                          ...
                          <View style={{ flex: 1 }}>
                            ...
                            <Text style={styles.hebType}>
                              {heb.type_hebergement}
                            </Text>
                          </View>
                          {hData && (
                            <Pressable
                              ...
                              onPress={() =>
                                router.push(
                                  `/accommodation/${hData._id}` as any,
                                )
                              }
                            >
